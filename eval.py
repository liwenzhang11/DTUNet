...
#预测结果路径
pred_path = r"E:\paper2\Doubleunet\shiyan_database\result2\all"
#标签路径
lab_path = r"E:\paper2\Doubleunet\database\all\png"
...
def tpcount(imgp,imgl):
    ...
    for i in range(WIDTH):
        for j in range(HIGTH):
            if imgp[i,j][1] == 255 and imgl[i,j] == 255:
                n = n+1
    return n
...
def fncount (imgp,imgl):
    ...
    for i in range(WIDTH):
        for j in range(HIGTH):
            if imgl[i,j] == 255 and imgp[i,j][1] == 0:
                n = n+1
    return n
...
def fpcount(imgp,imgl):
    ...
    for i in range(WIDTH):
        for j in range(HIGTH):
            if imgl[i,j] == 0 and imgp[i,j][1] == 255:
                n+=1
    return n
...
def tncount(imgp,imgl):
    ...
    for i in range(WIDTH):
        for j in range(HIGTH):
            if imgl[i,j] == 0 and imgp[i,j][1] == 0:
                n+=1
    return n
...
#准确率  accuracy
zq = (int(TN)+int(TP))/(int(TP)+int(FP)+int(TN)+int(FN))
#精确率    precision
