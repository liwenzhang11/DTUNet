...
def vnet_2d_base(input_tensor, filter_num, res_num_ini=1, res_num_max=5,
    ...
    * All the 5-by-5 convolutional kernels are changed (and fixed) to 3-by-3.
    ...
    X_skip = []
    X = input_tensor
    print("vnet_2d_base函数中输入的X:", K.int_shape(X))
    # ini conv layer
    ...
    X_skip.append(X) # 指的是vnet的跳跃连接，后面与deconv的输出进行cancat
    ...
    for i, f in enumerate(filter_num):
        X = vnet_right(X, [X_skip[i],], f, res_num=res_num_list[i],
                       activation=activation, unpool=False, batch_norm=batch_norm, name='{}_up_{}'.format(name, i))
...
def vnet_2d(input_size, filter_num, n_labels,
    ...
    * All the 5-by-5 convolutional kernels are changed (and fixed) to 3-by-3.
    ...
    OUT = Reshape((-1, n_labels))(OUT)
    print("shape of out", K.int_shape(OUT))
